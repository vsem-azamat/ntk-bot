...
class WeatherAPI:
    ...
    async def get_weather_forecast(self, days: int | None = None) -> dict:
        """
    ...
    async def __slice_zero_values(
        self, x: list, y: list, leave_extreme_zeros: bool = True
    ) -> tuple[np.ndarray, np.ndarray]:
        zero_indices = np.where(np.array(y) != 0)[0]

        if leave_extreme_zeros and len(zero_indices) > 0:
    ...
    async def __slice_time_interval(
        self, x: list[datetime], y: list, start_datetime: datetime, end_datetime: datetime
    ) -> tuple[np.ndarray, np.ndarray]:
        indices = np.where(
            np.logical_and(start_datetime <= np.array(x), np.array(x) <= end_datetime)
        )[0]
    ...
    async def plot_daily_weather_forecast(self) -> tuple[Figure, Axes, Axes]:
        ...
        x_datetimes, y_temperature_2m = await self.__slice_time_interval(
            datetimes, hourly["temperature_2m"], start_datetime, end_datetime
        )
        _, y_windspeed_10m = await self.__slice_time_interval(
            datetimes, hourly["windspeed_10m"], start_datetime, end_datetime
        )
        _, y_precipitation_probability = await self.__slice_time_interval(
            datetimes, hourly["precipitation_probability"], start_datetime, end_datetime
        )

        _, y_rain = await self.__slice_time_interval(
            datetimes, hourly["rain"], start_datetime, end_datetime
        )
        _, y_showers = await self.__slice_time_interval(
            datetimes, hourly["showers"], start_datetime, end_datetime
        )
        _, y_snowfall = await self.__slice_time_interval(
            datetimes, hourly["snowfall"], start_datetime, end_datetime
        )

        # Slice arrays from zero values
        x_datetimes_rain, y_rain = await self.__slice_zero_values(
            x_datetimes, y_rain, leave_extreme_zeros=True
        )
        x_datetimes_showers, y_showers = await self.__slice_zero_values(
            x_datetimes, y_showers, leave_extreme_zeros=True
        )
        x_datetimes_snowfall, y_snowfall = await self.__slice_zero_values(
            x_datetimes, y_snowfall, leave_extreme_zeros=True
        )

        # PLOT THE DATA
        # First diagram
        ax1.bar(x_datetimes, y_temperature_2m, color="gold", width=0.03, label="Temperature [ °C ]")
        for x, y in zip(x_datetimes, y_temperature_2m, strict=False):
        ...
        ax1.set_title(
            f"Weather for NTK: {start_datetime.strftime('%A')} {start_datetime.strftime('%d-%m-%Y')}"
        )
